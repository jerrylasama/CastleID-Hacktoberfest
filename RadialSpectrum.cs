...
public class RadialSpectrum : StoryboardObjectGenerator {
  ...
  public override void Generate() {
    ...
    var fftTimeStep =
        Beatmap.GetTimingPointAt(StartTime).BeatDuration / BeatDivisor;
    ...
    for (var i = 0; i < BarCount; i++) {
      ...
      bar.ColorHsb(StartTime, (i * 360.0 / BarCount) + Random(-10.0, 10.0),
                   0.6 + Random(0.4), 1);
      if (SpriteScale.X == SpriteScale.Y)
        bar.Scale(StartTime, barScale * SpriteScale.X);
      else
        bar.ScaleVec(StartTime, barScale * SpriteScale.X,
                     barScale * SpriteScale.Y);
      ...
      keyframes.ForEachPair(
          ...
          defaultPosition,
          s => new Vector2((float)Math.Round(s.X, CommandDecimals),
                           (float)Math.Round(s.Y, CommandDecimals)));
      if (!hasMove)
        bar.Move(StartTime, defaultPosition);
    }
  }
}
}
