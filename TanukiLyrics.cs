...
public class TanukiLyrics : StoryboardObjectGenerator {
  ...
  public void generateLyrics(FontGenerator font, SubtitleSet subtitles,
                             string layerName, bool additive) {
  ...
  public void generatePerLine(FontGenerator font, SubtitleSet subtitles,
                              StoryboardLayer layer, bool additive) {
    foreach (var line in subtitles.Lines) {
      var texture = font.GetTexture(line.Text);
      var position =
          new Vector2(320 - texture.BaseWidth * FontScale * 0.5f, SubtitleY) +
      ...
      foreach (var line in subtitleLine.Text.Split('\n')) {
        ...
        foreach (var letter in line) {
          ...
          if (!texture.IsEmpty) {
            ...
            var jarakdelay = (subtitleLine.EndTime - subtitleLine.StartTime) /
                             line.ToString().Length;

            switch (KaraokeEffect) {
            case KaraEffect.Default:
              sprite.Fade(subtitleLine.StartTime,
                          subtitleLine.StartTime + jarakdelay, 0, 1);
              sprite.Fade(subtitleLine.EndTime,
                          subtitleLine.EndTime + jarakdelay, 1, 0);
              break;
            case KaraEffect.Klasik:
              sprite.Move(OsbEasing.InOutExpo,
                          subtitleLine.StartTime + (jarakdelay * katake.Length),
                          subtitleLine.EndTime -
                              (jarakdelay * (line.Length - katake.Length + 1)),
                          position.X, position.Y, position.X, position.Y);
              break;
            case KaraEffect.Jalan:
              sprite.Fade(subtitleLine.StartTime, subtitleLine.StartTime, 0, 0);
              sprite.Fade(subtitleLine.StartTime + (jarakdelay * katake.Length),
                          (subtitleLine.StartTime +
                           (jarakdelay * (katake.Length + 2))) -
                              jarakdelay,
                          1, 0);
              break;
            case KaraEffect.JalanV2:
              sprite.Fade(subtitleLine.StartTime - (jarakdelay * katake.Length),
                          subtitleLine.StartTime -
                              (jarakdelay * katake.Length + 1),
                          1, 0);
              sprite.Fade(subtitleLine.EndTime -
                              (jarakdelay * (line.Length - katake.Length)),
                          subtitleLine.EndTime, 1, 0);
              break;
            case KaraEffect.Jajal:
              ...
              sprite.Rotate(
                  OsbEasing.InExpo,
                  subtitleLine.StartTime + (jarakdelay * katake.Length),
                  subtitleLine.StartTime + (jarakdelay * (katake.Length + 1)),
                  MathHelper.DegreesToRadians(Random(Math.PI * 2)),
                  MathHelper.DegreesToRadians(360));
              break;
            }

            if (additive)
              sprite.Additive(subtitleLine.StartTime - 200,
                              subtitleLine.EndTime);
          }
          if (VerticalMode) {
