...
def doFRS(username,password,datMK,index):
	...
	dataLogin = base64.b64encode(cipher.encrypt(username.encode() + b"|||" + password.encode()))
...
if __name__ == '__main__':
	...
	for i in range(int(config['MATKUL']['N'])):
		time.sleep(1)
		t = threading.Thread(target=doFRS, args=(username,password,config['MATKUL']['MK'+str(i)],i+1,))
		threads.append(t)
		t.start()